import { countFences, grouping, IdGenerator, load } from "./function.ts";
...
for (const [id, members] of Object.entries(groups)) {
  ...
  const fences = members.reduce((acc, m) => acc + countFences(m), 0);
  console.log({ id, region, fences });
  total = total + (region * fences);
